...
1. **Context size (primary)** — Reads the latest `usage` record from the session transcript and sums `input_tokens + cache_read_input_tokens + cache_creation_input_tokens` (the true context size of the turn). Suggests `/compact` at a window-scaled threshold — 160k tokens on a 200k window, 250k on a 1M window — and re-reminds after every additional 60k tokens of context growth.
...
- `cost-tracking` — Track spend across sessions
- `token-budget-advisor` — Control response depth and token use
