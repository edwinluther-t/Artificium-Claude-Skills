...
Integration reference for Plankton, a write-time code quality enforcement system for Claude Code. Plankton runs formatters and linters on every file edit via PostToolUse hooks, then spawns Claude subprocesses to fix violations the agent didn't catch.
...
Plankton's enforcement is implemented entirely through Claude Code's hook system:

- **PostToolUse hooks** — `multi_linter.sh` runs on every Write/Edit, triggering the three-phase quality loop
- **PreToolUse hooks** — `protect_linter_configs.sh` blocks config file edits before they happen
- **Stop hooks** — `stop_config_guardian.sh` catches any config drift via `git diff` at session end

To integrate into your own project, wire these hooks in `.claude/settings.json` under the appropriate `postToolUse`, `preToolUse`, and `stop` keys.

## Language Gate Defaults
...
During quality enforcement, flag changes to config files in the same iteration:

- `biome.json`, `.eslintrc*`, `prettier.config*`, `tsconfig.json`, `pyproject.toml`

If a config is changed to suppress violations, require explicit review before merge.
