...
each phase to the matching agent or command. This file is that pipeline.
...
> feature development, planning, code review, build fixes, refactoring, and
> the GAN build loop, plus the `tdd-workflow` skill. The orch-* family adds the
> shared size classifier and the two gates on top of them, so one umbrella
> covers all five operations consistently.
...
- **1. Research & Reuse** — `gh search repos` / `gh search code`, then library
  docs, then package registries, then web search. Prefer adopting a proven
  implementation over net-new code.
...
- **4. Implement (TDD)** — drive each task through the `tdd-guide` agent (or the
  `tdd-workflow` skill): red → green → refactor. Honor the operation's
  first-move rule.
- **5. Review** — `code-reviewer` agent. Add `security-reviewer` whenever the
  diff touches a security trigger (below).
...
| Implement | `tdd-guide` (or `tdd-workflow` skill) | `build-error-resolver` on build breaks |
| Review | `code-reviewer` | language reviewer (`python-reviewer`, `typescript-reviewer`, …) |
| Security | `security-reviewer` | — |
| MVP inner loop | GAN build loop `--skip-planner` | drives generator → evaluator; tune `--max-iterations` / `--pass-threshold` |
...
external API calls, cryptography, or secrets / credentials.
