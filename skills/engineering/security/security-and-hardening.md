...
Pair with `SameSite=Strict` on all cookies. For SPAs, the double-submit cookie pattern is an alternative.
...
Test security properties as first-class behaviors:
...
// Test input validation at boundary
...
For offensive vulnerability discovery and bounty submission, see `security-bounty-hunter`.
