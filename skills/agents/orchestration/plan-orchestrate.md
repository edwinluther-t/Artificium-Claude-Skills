...
description: Read a plan document, decompose it into steps, design a per-step agent chain from the available catalogue, and emit ready-to-paste orchestrate custom prompts. Generative only — never invokes orchestrate itself. Use when the user has a multi-step plan and wants to drive it through orchestrate without composing chains by hand.
---
...
- `docs-lookup` — third-party library API lookups
...
2. Detect install form once and freeze it into `INSTALL_MODE`. Algorithm (run in order, stop at the first match):
   1. If a plugin marketplace directory exists under the Claude home directory → `INSTALL_MODE=plugin`.
   2. Else if an agents directory exists with at least one agent file (e.g. `tdd-guide.md`, `code-reviewer.md`) → `INSTALL_MODE=legacy`.
   3. Else → default to `INSTALL_MODE=legacy` and emit a one-line warning: `> Warning: could not detect install; defaulting to legacy form. If you use the plugin install, edit the prefixes manually.`
   4. If both markers exist (mixed install), `plugin` wins.
...
5. **Normalize any agent names declared in the plan**: if the plan text references agents by their prefixed form, strip the prefix to get the bare catalogue name before validating or composing chains. Re-prefixing happens only at output time per `INSTALL_MODE`. Never let a pre-prefixed name flow into chain composition — it would double-prefix in plugin mode.
...
10. Steps tagged `impl`, `refactor`, or `migration` end with a **reviewer-class** agent — any of `<lang>-reviewer`, `code-reviewer`, `security-reviewer`, or `database-reviewer`. The most domain-specific reviewer wins the tail position. `test` and `build` steps are gated by their own validators (`e2e-runner` and the build resolver respectively) and do not require an additional reviewer.
...
Emit Markdown using the form determined by `INSTALL_MODE`. The output uses one form throughout — every slash command and every agent name is rendered with the matching prefix from Phase 0. **Do not emit both forms; do not include "this is plugin form" / "strip the prefix" instructions in the rendered output.**
...
- [ ] All slash commands and agent names use the **same** form throughout (`plugin` or `legacy`) — never mixed in one output.
- [ ] No install-form annotations and no "strip the prefix" instructions remain in the rendered output.
...
- **Plan declares agents** (rare): first strip any namespace prefix to get the bare catalogue name, then validate against the catalogue. Replace invalid agents and explain under "Chain rationale". The bare name is re-prefixed at output time per `INSTALL_MODE`.
